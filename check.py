...
def check(ip,version):
    b=commands.getstatusoutput("sshpass -p "+cpassw.cpass()+" ssh {} rpm -q jdk".format(ip))
    d=b[1]
    i=d[0:3]
#    print i
    if i == "jdk":
...
#        else :
    ...
    c=commands.getstatusoutput("sshpass -p "+cpassw.cpass()+" ssh {} hadoop version | grep 'Hadoop'".format(ip))
    h=c[1]
    g=h[0:6]
#    print g
    if g=="Hadoop":
...
#	commands.getstatusoutput("ssh {} python setpath.py".format(ip))
	return 1
def dialog():
	o=commands.getstatusoutput("rpm -q dialog")
	out=o[1]
	if out[0:6]=="dialog":
		
		print "dialog is already installed"
	else :
		print "no dialog is installed"	
		install.dialog()
def sshpass():
	o=commands.getstatusoutput("rpm -q sshpass")
	out=o[1]
	if out[0:7]=="sshpass":
		
		print "sshpass is already installed"
	else :
		print "no sshpass is installed"	
		install.sshpass()
